...
const JWT_SECRET='Mounuisagood$girl';
...
try {
    ...
    if(!user){
        return res.status(400).json({error:"plz try to login with crct dtls"});
    }
    const passwordCompare= await bcrypt.compare(password,user.password);
    if(!passwordCompare){
        success=false;
        return res.status(400).json({ success,zerror:"plz try to login with crct dtls"});
    }
    const data={
        user:{
            id:user.id
